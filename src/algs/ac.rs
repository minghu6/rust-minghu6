...
fn build_ac_automation(target_strings:Vec<&str>) -> *mut TrieTree {
    let tree = TrieTree::new();

    for target in target_strings.iter() {
        let mut node = unsafe { &mut *tree.root_ptr };
                ...
                while !children_sibling_ptr.is_null() {
                    ...
                    loop {
                        if failed_ptr.is_null() {
                            children_sibling.failed_ptr = tree.root_ptr;
                            break;
                        }
...
pub fn index_of_multistrings<'a>(keys: Vec<&'a str>, text: &str) -> BTreeMap<&'a str, Vec<usize>>{
    let mut result:BTreeMap<&'a str, Vec<usize>> = BTreeMap::new();
    for s in keys.iter() {
        result.insert(*s, vec![]);
    }

    let tree_ptr = build_ac_automation(keys);
    let root = unsafe { &mut *(*tree_ptr).root_ptr };

    let mut state_node = &mut *root;
...
mod tests {
    ...
    fn ac_automation_works() {
        let mut result = index_of_multistrings(
            vec!["bcd", "cdfkcdf", "cde", "abcdef", "abhab"], 
        ...
        result = index_of_multistrings(
            vec!["she", "shr", "say", "he", "her"],
    ...
    fn bench_ac(b: &mut Bencher) {
        ...
        b.iter(||index_of_multistrings(keys.clone(), text_contents.as_str()));
    }
