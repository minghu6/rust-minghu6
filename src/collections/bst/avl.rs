...
#[inline]
fn aavl_height<K: BSTKey, V>(x: &AavlNodeRc<K, V>) -> usize {
    ...
    aavl_height_(&Some(x.clone())) as usize
}
...
impl<K: BSTKey, V> Aavl<K, V> {
    ...
    fn delete_retracing(&mut self, removed_node: &AavlNodeRc<K, V>) {
...
impl<K: BSTKey, V> Adictionary<K, V> for Aavl<K, V> {
    ...
    fn remove(&mut self, key: &K) -> Option<Arc<V>> {
        ...
        self.delete_retracing(&approxi_node);
...
mod tests {
    ...
    use super::Aavl;
    use crate::test::dict::{DictProvider, Inode, InodeProvider};
    #[test]
