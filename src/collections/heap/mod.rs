...
macro_rules! test_heap {
    ...
    ($heap:expr, $endian:ident, push:$push:ident, pop:$pop:ident) => {
        use $crate::etc::gen_unique;
        let get_one = || rand::random::<u64>();

        let non_dec = $crate::collections::heap::heap_endian_no_dec!($endian);
        ...
        for _ in 0..20 {
            ...
            let mut refheap = $crate::collections::heap::union_heap!($endian);
...
macro_rules! test_heap_update {
    ($heap:expr, $endian:ident) => {
        test_heap_update!($heap, $endian, push:push, pop:pop, update:update);
    };
    ($heap:expr, $endian:ident, push:$push:ident, pop:$pop:ident, update:$update:ident) => {
        #[allow(unused)]
        use $crate::collections::{ Coll, AdvHeap };
        let get_one = || rand::random::<u64>();
        let non_dec = $crate::collections::heap::heap_endian_no_dec!($endian);
        ...
        for _ in 0..400 {
            ...
            for _ in 0..batch_num / 2 {
                let newkey = get_one();
                let i = crate::algs::random_range(0..testheap.len());
...
macro_rules! union_heap {
    (MAX) => {
        {
            $crate::collections::heap::MaxDictHeap::new()
        }
    };
    (MIN) => {
        {
            $crate::collections::heap::MinDictHeap::new()
        }
    }
}
...
#[cfg(test)]
pub(super) use test_heap;
#[cfg(test)]
pub(super) use test_heap_update;
#[cfg(test)]
pub(super) use heap_endian_no_dec;
#[cfg(test)]
pub(super) use union_heap;
